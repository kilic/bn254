...
func (e *fp6) fromBytes(b []byte) (*fe6, error) {
	if len(b) < 192 {
		return nil, errors.New("input string should be larger than 192 bytes")
	}
	fp2 := e.fp2
	u2, err := fp2.fromBytes(b[:64])
	if err != nil {
		return nil, err
	}
	u1, err := fp2.fromBytes(b[64:128])
	if err != nil {
		return nil, err
	}
	u0, err := fp2.fromBytes(b[128:])
	if err != nil {
		return nil, err
	}
...
func (e *fp6) toBytes(a *fe6) []byte {
	fp2 := e.fp2
	out := make([]byte, 192)
	copy(out[:64], fp2.toBytes(&a[2]))
	copy(out[64:128], fp2.toBytes(&a[1]))
	copy(out[128:], fp2.toBytes(&a[0]))
	return out
}
